...
def eval_epoch(model, validation_data, device, opt):
    ...
    desc = "  - (Validation) "
    with torch.no_grad():
        for img_tensor, target, img_name in tqdm(
            validation_data, mininterval=2, desc=desc, leave=False
        ):
...
def train(model, train_data, val_data, optimizer, device, cfg, data):
    ...
    print(
        f"[+] Training performance will be written to file: {log_train_file} and {log_valid_file}"
    )
    ...
    def print_performances(header, ppl, accu, start_time, lr):
        ...
        print(
            f"- {header:12} ppl: {ppl: 8.5f}, accuracy: {accu:3.3f}, "
    ...
    for epoch_i in range(cfg["EPOCHS"]):
        ...
        checkpoint = {'epoch': epoch_i, 'settings': cfg,
        ...
        if cfg["SAVE_MODE"] == 'all':
            ...
            if valid_loss <= min(valid_losses):
                torch.save(
                    checkpoint, os.path.join(cfg["SAVE_DIR"], model_name)
                )
                print('    - [Info] The checkpoint file has been updated.')

        with open(log_train_file, 'a') as log_tf, open(log_valid_file, 'a') as log_vf:
            log_tf.write(
                f'{epoch_i},{train_loss: 8.5f},{train_ppl: 8.5f},{100 * train_accu:3.3f}\n'
            )
            log_vf.write(
                f'{epoch_i},{valid_loss: 8.5f},{valid_ppl: 8.5f},{100 * valid_accu:3.3f}\n'
            )
        ...
        if cfg["TENSOR_BOARD"]:
            ...
            tb_writer.add_scalars(
                'accuracy', {'train': train_accu * 100,
                             'val': valid_accu * 100}, epoch_i
            )
